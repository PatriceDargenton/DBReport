...
Private Sub ShowMessageDeleg(sender As Object, e As DatabaseSchemaReader.ReaderEventArgs) _
    ...
    End If
    ShowMsg("Reading database schema : " & e.ProgressType.ToString & ", " & _
        e.SchemaObjectType.ToString) ' & ", " & e.Name & ", " & e.Index & ", " & e.Count)
...
Public Function bCreateDBReport(prm As clsPrmDBR, delegMsg As clsDelegMsg, _
    ...
    Try
        ...
        If bMySql Then
            If Not bGetMySqlParameters(prm.sConnection, prm.sDBName, dicSqlPrm, lstMySqlPrm, _
...
Private Sub ShowMySqlInfos(sb As StringBuilder, prm As clsPrmDBR, _
    ...
    For Each sPrm In lstMySqlPrm
        ...
        If sPrm = enumMySqlPrm.net_read_timeout_sec OrElse _
...
Private Sub CreateTableReport(sb As StringBuilder, prm As clsPrmDBR, _
    schema As DatabaseSchemaReader.DataSchema.DatabaseSchema, bMySql As Boolean, _
    dicSqlTE As Dictionary(Of String, String), _
    dicSqlTC As Dictionary(Of String, String), _
...
Private Sub CreateLinkReport(sb As StringBuilder, prm As clsPrmDBR, _
...
Retry:          ' 04/09/2016 A constraint may be duplicated
        ...
        For Each fk0 In dicLinks.Sort(sSorting)
            ...
            If fk.DeleteRule <> sRestrictDef Then sDelRule = " (Delete rule : " & _
                CStr(IIf(String.IsNullOrEmpty(fk.DeleteRule), "undefined", fk.DeleteRule)) & ")"
            sRestrictDef = prm.sForeignKeyUpdateRuleDef
            If fk.UpdateRule <> sRestrictDef Then sUpdRule = " (Update rule : " & _
...
Public Function bGetMySqlParameters(sMySQLConnectionString$, sDbName$, _
    dicMySqlPrm As Dictionary(Of String, String), lstMySqlPrm As List(Of String), _
...
Public Sub GetMySqlTablesCollationAndEngine(sMySQLConnectionString$, sDbName$, _
    dicMySqlTE As Dictionary(Of String, String), _
    ...
    Try
        ...
        Using oConnMySQL As New MySql.Data.MySqlClient.MySqlConnection
            ...
            Dim lstSQL As New List(Of String)
            lstSQL.Add("SELECT TABLE_NAME, ENGINE, COLLATION_NAME FROM information_schema.`TABLES` T," & _
                " information_schema.`COLLATION_CHARACTER_SET_APPLICABILITY` CCSA" & _
...
Public Sub GetMySqlColumnsCollation(sMySQLConnectionString$, sDbName$, _
    ...
    Try
        ...
        Using oConnMySQL As New MySql.Data.MySqlClient.MySqlConnection
            ...
            Dim lstSQL As New List(Of String)
            lstSQL.Add("SELECT table_name, C.column_name, COLLATION_NAME FROM " & _
